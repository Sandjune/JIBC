...
# JIBC Workshops Navigator (updated v7)
# - Auto-installs missing deps (python-docx, pymupdf) at runtime and reruns the app
# - PDF display modes: Auto (Images → Data-URI → File URL), Images only, Data-URI only, File URL only
# - Image zoom slider; viewer height slider
...
# Try imports (may be None if not installed yet)
try:
    import fitz  # PyMuPDF
except Exception:
    fitz = None
...
def ensure_dependency(pip_name: str, import_name: str) -> (bool, Optional[str]):
    """Ensure a dependency is installed; try to import, else pip install, then import again."""
    try:
...
def pdf_to_images(pdf_path: Optional[str] = None, pdf_bytes: Optional[bytes] = None, zoom: float = 2.0) -> List[bytes]:
    ...
    # Import fitz lazily (after ensure_dependency)
    global fitz
    if fitz is None:
        try:
...
def extract_sections_from_docx(docx_path: Optional[str] = None, docx_bytes: Optional[bytes] = None) -> Dict[str, str]:
    ...
    # Import python-docx lazily (after ensure_dependency)
    global DocxDocument
...
def diagnostics(pdf_path: Optional[str], docx_path: Optional[str], pdf_bytes: Optional[bytes], docx_bytes: Optional[bytes]):
    ...
    # Check imports live rather than cached state
    docx_ok = False
    fitz_ok = False
    try:
    ...
    try:
        ...
        pass
    st.sidebar.write(f"**python-docx available:** {'✅' if docx_ok else '❌'}")
    st.sidebar.write(f"**PyMuPDF (fitz) available:** {'✅' if fitz_ok else '❌'}")
...
def main():
    ...
    # One-time dependency bootstrap (optional)
    if not st.session_state.get("deps_bootstrapped", False):
        docx_ok, docx_err = ensure_dependency("python-docx", "docx")
        fitz_ok, fitz_err = ensure_dependency("pymupdf", "fitz")
        st.session_state["deps_bootstrapped"] = True
        # If at least one new dep was installed, rerun to load modules
        if (docx_ok and DocxDocument is None) or (fitz_ok and (globals().get("fitz") is None)):
            st.rerun()
